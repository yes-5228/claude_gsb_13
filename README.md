...
| 保洁巡查 | `/inspections` | 8 项检查项打分、自动折算百分制得分与等级、班次/日期/结论/巡查人筛选、巡查详情、巡查人更正留痕、一键转问题上报 |
...
| POST | `/inspections/{id}/inspector-corrections` | 更正巡查人（须填更正原因，原巡查人留痕，得分与结论不变） |
| GET | `/inspections/{id}/inspector-corrections` | 巡查人更正记录列表 |
...
- **巡查人更正**：巡查人填错时可通过更正接口修正，必须填写更正原因；每次更正都会留存更正前后的巡查人与原因（详情页展示更正记录），得分与结论不因更正而变化；列表按巡查人筛选时，更正前后的姓名都能检索到该记录。
...
- 后端 `pytest`：7 个用例全部通过；`/health`、台账/巡查/问题/统计/字典接口均返回预期数据。
