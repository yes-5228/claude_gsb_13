...
| POST | `/inspections/{id}/inspector-correction` | 更正巡查人（必填更正原因，留存原巡查人；得分、等级、结论不变） |
...
- **巡查人更正**：巡查人填错时通过专用更正接口修改，必须填写更正原因；系统留存原巡查人、更正后巡查人、原因、操作人与更正时间，形成不可覆盖的更正流水。**更正只改变巡查人，不重新算分、不改变等级与结论**；巡查人筛选/关键字检索同时命中当前巡查人与历史原巡查人，详情页时间线可查每次更正的前后人员与原因，列表对已更正记录加「已更正」标记。通用 `PATCH /inspections/{id}` 不再受理巡查人字段，防止绕过审计静默修改。
